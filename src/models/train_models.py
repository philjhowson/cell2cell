from training_utils import train_log_rf, train_xgb, load_data
from general_utils import safe_saver
import os
import json
import argparse

def train_models(model, sample, strategy):
    """
    ...
    strategies = set(['baseline', 'rfecv', 'pca'])
    ...
    """
    X_train, y_train, X_test, y_test = load_data(model, sample, strategy)

    if model != 'xgb':
        classifier, params, results, features = train_log_rf(params, X_train, y_train, X_test, y_test, model)

    else:
        classifier, params, results, features = train_xgb(params, X_train, y_train, X_test, y_test)    

    """
    Saves the model, best parameters, results, feature importances, and pso results if pso was selected.
    """

    safe_saver(classifier, f"models/{sample}", f"{strategy}_{model}_model")
    with open(f"models/{sample}/{strategy}_{model}_params.json", 'w') as f:
        json.dump(params, f)
    safe_saver(params, f"models/{sample}/", f"{strategy}_{model}_params")

    os.makedirs(f"metrics/{sample}/", exist_ok = True)
    with open(f"metrics/{sample}/{strategy}_scores_{model}.json", 'w') as f:
        json.dump(results, f)

    features.to_csv(f"models/{sample}/{strategy}_feature_importances_{model}.csv")   
...
if __name__ == '__main__':
    ...
    parser.add_argument('--strategy', required = True, help = 'baseline, pca, or rfecv.')

    arg = parser.parse_args()

    train_models(arg.model, arg.sample, arg.strategy)